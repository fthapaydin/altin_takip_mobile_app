...
class CalculatorProfitTab extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<CalculatorProfitTab> createState() => _CalculatorProfitTabState();
}
...
class _CalculatorProfitTabState extends ConsumerState<CalculatorProfitTab> {
  ...
  double _calculateProfit() {
    final buyPrice = double.tryParse(_buyPriceController.text.replaceAll('.', '').replaceAll(',', '.')) ?? 0;
    final sellPrice = double.tryParse(_sellPriceController.text.replaceAll('.', '').replaceAll(',', '.')) ?? 0;
  ...
  Widget build(BuildContext context) {
    ...
    final buyPrice = double.tryParse(_buyPriceController.text.replaceAll(',', '.')) ?? 0;
    final sellPrice = double.tryParse(_sellPriceController.text.replaceAll(',', '.')) ?? 0;
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
            ...
            onSelected: (currency) {
              setState(() {
                ...
                if (currency != null) {
                  ...
                  _sellPriceController.text = formatter.format(currency.selling);
                }
              });
            },
          ),
          ...
          const Gap(32),
          _buildResultCard(profit, percentage, isPositive, themeColor, buyPrice, sellPrice, amount),
          const Gap(120),
        ],
          ...
          Expanded(
            child: Column(
              ...
              children: [
                Row(
                  children: [
                    Icon(isPositive ? Iconsax.trend_up : Iconsax.trend_down, color: themeColor, size: 16),
                    const Gap(8),
                    Text(
                ...
                if (amount > 0) ...[
                  ...
                  const Gap(16),
                  _buildProfitDetail('TOPLAM MALİYET', '₺${NumberFormat('#,##0.00', 'tr_TR').format(buyPrice * amount)}'),
                  const Gap(8),
                  _buildProfitDetail('TOPLAM DEĞER', '₺${NumberFormat('#,##0.00', 'tr_TR').format(sellPrice * amount)}'),
                ],
              ],
            ),
          ),
